...
#pragma omp for
        for (int w = 1; w <= nrOfIds; w++) {
            for (int q = 1; q <=nrOfPhotosPerId; q++) { //get all the file's names
             //  cout << "Nr of threads = " << omp_get_thread_num() << endl;
                filename = to_string(w) + "." + to_string(q) + ".txt";


                int **image = read_pgm_file(filename, num_rows, num_cols);
                int **img = alloc_2d_matrix((num_rows + 2),
                                            (num_cols + 2)); //enhanced image matrix with 0 in the corners
